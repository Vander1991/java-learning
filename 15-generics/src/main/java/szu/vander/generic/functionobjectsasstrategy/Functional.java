package szu.vander.generic.functionobjectsasstrategy;
 ...
 * @author     : Vander
 * @date       : 2019-01-03
 * @description: 为不同类型的函数对象定义接口，这些接口都是按需创建，为每个接口实现了不同方法。
 * 				在C++中，潜在类型机制将在你调用函数时负责协调各个操作，但是在Java中，我们需要编写函数对象来将泛型方法适配
 * 				为我们特定的需求。这个类接下来的部分展示了函数对象的各种不同的实现。例如，IntegerAdder和BigDecimalAdder
 * 				通过为它们特定的类型调用恰当的方法，从而解决了相同的问题，即添加两个对象，因此这是适配器模式和策略模式的结合
 */
