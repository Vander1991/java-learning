# 21-并发：
	...
	终结任务
		...
		在阻塞时终结
			线程的四种状态
				1、新建（new）：当线程被创建，它只会短暂地处于这种状态，此时它已经分配了必须的系统资源，并执行了初始化，此刻线程已经有资格获得
			CPU时间了，之后调度器将把这个线程转变为可运行的状态或阻塞状态。
				2、就绪（Runnable）：在这种状态下，只要调度器把时间片分配给线程，线程就可以运行。即在任意时刻，线程可以运行也可以不运行。只
			要调度器能分配时间片给线程，它就可以运行，这不同于死亡和阻塞状态。
				3、阻塞（Blocked）：线程能够运行，但是某个条件阻止了它的运行。当线程处于阻塞状态时，调度器将忽略线程，不会分配给线程CPU时间。
			直到线程重新进入就绪状态，它才有可能执行操作。
				4、死亡（Dead）：处于死亡或终止状态的线程将不再是可调度的，并且再也不会得到CPU时间，它的任务已结束，或不再是可运行的。任务死亡
			的通常方式是从run()方法返回，但是任务的线程还可以被中断。
			进入阻塞状态
				一个任务进入阻塞状态，可能有如下原因：
				1）通过调用sleep(ms)使任务进入休眠状态，在这种情况下，任务在指定时间内不会运行。
				2）通过调用wait()使线程挂起，知道线程得到了notif()或notifyAll()消息（JDK5中的concurrent类库中等价的signal()或
				signalAll()消息），线程才会进入就绪状态。
				3）任务在等待某个输入/输出完成
				4）任务试图在某个对象上调用其同步控制方法，但是对象锁不可用，因为另一个任务已经获得了这个锁。
			
			中断
				在Runnable.run()方法的中间打断它，比正常退出或者是代码执行到你设定的某个标记然后退出要棘手得多，因为你打断被阻塞的任务的时候，
			可能还需要清理资源，在任务的run()方法中间打断，更像是抛出的异常，因此在Java线程中的这种类型的异常中断中用到了异常（这回滑向异常的不恰当
			用法，因为这意味着你经常用它们来执行流程控制）。为了以这种方式终止任务，你需要考虑代码的执行路径然后在catch中清理资源。
				Thread类中包含interrupt()方法，用来终止被阻塞的任务，这个方法将设置线程的中断状态，如果一个线程已经被阻塞，或者准备执行一个
			阻塞操作，那么设置这个线程的中断状态将抛出InterruptedException。当抛出该异常或者该任务调用Thread.interrupted()时，中断
			状态将被复位。Thread.interrupted()提供了离开run()循环而不抛出异常的第二种方式。
				为了调用interrupt()，似乎要用到Thread对象，但是新的concurrent类库似乎在避免你直接操作Thread对象，JDK想让你尽量使用
			Executor来执行所有操作。如果用exec来执行shutdownNow()，它将发送一个interrupt()调用给它启动的所有线程。当你完成工程里的某个
			部分或者整个程序时，通常会希望同时关闭某个特定Executor的所有任务。然而，你有时也会希望只中断某个特定任务。如果使用Executor，那么通过
			调用submit()而不是executor()来启动任务，就可以持有该任务的上下文。submit()将返回一个泛型Future<?>，其中有一个未修饰的参数，
			因为你永远都不会在其上调用get()——持有这种Future的关键在于你可以在其上调用cancel()，并因此可以使用它来中断某个特定任务。如果你将
			true传递给cancel()，那么它就会拥有停止这个线程的权限（该线程上调用interrupt()就能停止这个线程），因此cancel()是一种中断由
			Executor启动单个线程的方式。
				Sleep而阻塞的线程是可以中断的，但是由于IO、Synchronized阻塞的线程是不能中断的，能中断的类型只有在代码中要求抛出Interrupt
			Exception的。但是在Web程序中的确想关闭IO操作造成的阻塞应该要怎么办呢，有个方法有点笨，就是关闭任务在其上发生阻塞的底层资源（见Close
			Resource类）。
				nio类提供了更人性化的I/O中断，被阻塞的nio通道会自动响应中断（见NIOInterruption类）
				
				被互斥所阻塞
					如果尝试着在一个对象上调用其synchronized方法，而这个对象的锁已经被其他任务获得，那么调用任务将被挂起（阻塞），直至这个锁可
				获得。就像前面在不可中断的I/O中所观察到的那样，无论在任何时刻，只要任务以不可中断的方式被阻塞，那么就有锁住程序的潜在可能。JDK5
				并发类库增加了一个特性，即在ReentrantLock上阻塞的任务具备可以被中断的能力，这与在synchronized方法或临界区上阻塞的任务完全
				不同。
				
				检查中断
					能够接收中断的时间只能是在阻塞的时候，所以如果写出了会阻塞的程序，在阻塞时调用中断要保证资源能够正确被清理，但是也要同时考虑
				在没有被阻塞的情况下，程序接收到中断信号，但是程序不会中断，而是会继续执行的情况下，资源也得正确地被清理。
					方法是：所有需要清理的对象创建操作后面，都必须紧跟try-finally子句，从而使得无论run()循环如何退出，清理都会发生。在C++中
				这个清理操作一般是在析构函数中执行的。
	
	线程之间的协作
		有时候需要任务与任务之间进行协作，例如一个任务执行完了之后需要告诉另一个任务去继续执行下去。关键的问题是它们之间要怎么握手，这里也是使用相同的
	特性：互斥。互斥可以确保只有一个任务可以响应某个信号，这样就可以根除任何可能的竞争条件。在互斥之上，我们为任务添加了一种途径，可以将其自身挂起，直到
	外部条件告知它可以继续运行下去了。任务间的握手可以通过Object的方法wait()和notify()来安全地实现，JDK5中并发类库还提供了await()和signal()
	方法的Condition对象。
		
		wait()和notifyAll()
			wait()会等待外部产生变化的时候将任务挂起，并且只有notify()或notifyAll()发生时，这个任务才会被唤醒并去检查所产生的变化。
			实际上，调用sleep()、yield()的时候，锁并没有被释放，而使用wait()则会将锁释放。所以调用wait()的时候，就像在声明：“我已经刚刚做完能
		做的所有事情，因此我要在这里等待，但是我希望其他的synchronized操作在条件合适的情况下能够执行。”
			wait()的两种版本：
				1、接受毫秒为参数，在多少毫秒内挂起，它与sleep的不同在于它的锁是释放的，还有可以通过notify、notifyAll或时间到了，任务就能恢复
				运行。
				2、不接受任何参数，这种比较常用。这样wait会一直等待下去，直到线程接收到notify、notifyAll信号。
				
				wait、notify、notifyAll这三个方法都不是Thread的方法，而是Object的方法，因为这些方法操作的锁也是Object的一部分，所以这样
			设计也合理。只能将这三个方法放在同步控制的方法里，如果放在非同步的方法里，会抛出IllegalMonitorStateException异常，并告知你“当前
			线程不是拥有者”，意思是这三个方法的任务在调用这些方法前必须拥有对象的锁。也可以让另一个对象执行某种操作以维护自己的锁，这么做的话首先要获
			得对象的锁，比如，如果要向对象x发送notifyAll，那么就必须在能够取得x的锁的同步控制块中这么做：
				synchronized(x) {
					x.notifyAll();
				}
				正确的使用notify和wait的本质是，要检查所感兴趣的特定条件，并在条件不满足的情况下返回wait()中，惯用的方法就是使用while来编写这
			种代码。
